...
describe('ImageOverview.vue', () => {
  ...
  test('template: header-left button disabled/enabled and sub-row renders when grouped (full mount)', async () => {
    ...
    (wrapper.vm as any).rows = [ { id: '1', imageMetadata: { repository: 'repo', registry: 'r' }, metadata: { name: 'n' }, report: { summary: { critical: 1 } } } ];
  ...
  test('renders and downloadCSVReport calls saveAs and growl on success', async () => {
    ...
    const rows = [ {
      ...
      imageMetadata: { registryURI: 'reg', repository: 'repo', tag: 'tag', digest: 'sha256:abc', registry: 'reg', platform: 'linux/amd64' },
      report: { summary: { critical: 1, high: 0, medium: 0, low: 0, unknown: 0 } },
      scanResult: { critical: 1, high: 0, medium: 0, low: 0, unknown: 0 },
      metadata: { name: 'img1' }
    } ];
    ...
    const vulReports = [
      {
        ...
        report: { summary: { critical: 1, high: 2, medium: 0, low: 0, unknown: 0 } }
      },
      {
        id: '2',
        imageMetadata: { repository: 'repo1', registry: 'reg1' },
        metadata: { name: 'img-b', namespace: 'ns' },
        report: { summary: { critical: 0, high: 1, medium: 0, low: 0, unknown: 0 } }
      }
    ];
  ...
  test('customActions invoke download handlers', async () => {
    ...
    const wrapper = mountComponent({ global: { mocks: { $store: store, $t: t, t } } });
  ...
  (wrapper.vm as any).rows = [
      { imageMetadata: { registryURI: 'r1', repository: 'repo', tag: 't1', registry: 'reg' }, report: { summary: { critical: 1, high: 0, medium: 0, low: 0, unknown: 0 } }, metadata: { namespace: 'ns' } },
      { imageMetadata: { registryURI: 'r2', repository: 'other', tag: 't2', registry: 'reg' }, report: { summary: { critical: 0, high: 0, medium: 0, low: 0, unknown: 0 } }, metadata: { namespace: 'ns' } },
    ];
  ...
  test('filteredRows imageName match uses registryURI/repository:tag construction', () => {
    ...
    (wrapper.vm as any).rows = [
      { imageMetadata: { registryURI: 'rX', repository: 'myrepo', tag: 'ver1', registry: 'reg' }, report: { summary: { critical: 0, high: 0, medium: 0, low: 0, unknown: 0 } }, metadata: { namespace: 'ns' } },
      { imageMetadata: { registryURI: 'other', repository: 'repo2', tag: 'v2', registry: 'reg' }, report: { summary: { critical: 0, high: 0, medium: 0, low: 0, unknown: 0 } }, metadata: { namespace: 'ns' } },
    ];
  ...
  test('filteredRows registrySearch matches namespace/registry string', () => {
    ...
    (wrapper.vm as any).rows = [
      { imageMetadata: { registryURI: 'r1', repository: 'repoA', tag: 't', registry: 'regA' }, report: { summary: { critical: 0, high: 0, medium: 0, low: 0, unknown: 0 } }, metadata: { namespace: 'nsA' } },
    ];
  ...
  test('filteredRows platformMatch uses imageMetadata.platform substring match', () => {
    ...
    (wrapper.vm as any).rows = [
      { imageMetadata: { registryURI: 'r2', repository: 'repoB', tag: 't2', registry: 'regB', platform: 'linux/amd64' }, report: { summary: { critical: 0, high: 0, medium: 0, low: 0, unknown: 0 } }, metadata: { namespace: 'nsB' } },
      { imageMetadata: { registryURI: 'r3', repository: 'repoC', tag: 't3', registry: 'regC', platform: null }, report: { summary: { critical: 0, high: 0, medium: 0, low: 0, unknown: 0 } }, metadata: { namespace: 'nsC' } },
    ];
  ...
  test('downloadCSVReport handles papaparse throwing and dispatches error', async () => {
    ...
    const rows = [ {
      ...
      imageMetadata: { registryURI: 'reg2', repository: 'repo2', tag: 'tag2', digest: 'sha256:ddd', registry: 'reg2', platform: 'linux/amd64' },
      report: { summary: { critical: 0, high: 0, medium: 0, low: 0, unknown: 0 } },
      scanResult: { critical: 0, high: 0, medium: 0, low: 0, unknown: 0 },
      metadata: { name: 'img2' }
    } ];
  ...
  test('filteredRows repositorySearch filters by repository value', () => {
    ...
    (wrapper.vm as any).rows = [
      { imageMetadata: { registryURI: 'rA', repository: 'special', tag: 't', registry: 'regA' }, report: { summary: { critical: 0, high: 0, medium: 0, low: 0, unknown: 0 } }, metadata: { namespace: 'nsA' } },
      { imageMetadata: { registryURI: 'rB', repository: 'other', tag: 't2', registry: 'regB' }, report: { summary: { critical: 0, high: 0, medium: 0, low: 0, unknown: 0 } }, metadata: { namespace: 'nsB' } },
    ];
  ...
  test('downloadSbom success and error flows', async () => {
    ...
    const wrapperSuccess = mountComponent({ global: { mocks: { $store: storeSuccess, $t: t, t } } });
    ...
    const wrapperError = mountComponent({ global: { mocks: { $store: storeError, $t: t, t } } });
  ...
  test('downloadJson success and error flows', async () => {
    ...
    const wrapper = mountComponent({ global: { mocks: { $store: storeSuccess, $t: t, t } } });
    await (wrapper.vm as any).downloadJson([ { id: 'v1' } ]);
    expect(fileSaver.saveAs).toHaveBeenCalled();
    expect(storeSuccess.dispatch).toHaveBeenCalledWith('growl/success', expect.any(Object), { root: true });

    const storeError = { dispatch: jest.fn((action: string) => {
      if (action === 'cluster/find') {
        throw new Error('bad');
      }
      return Promise.resolve();
    }), getters: {} };
    const wrapperErr = mountComponent({ global: { mocks: { $store: storeError, $t: t, t } } });
  ...
  test('downloadCsv success and error flows', async () => {
    const vulReport = {
      report: {
        results: [ { vulnerabilities: [ { cve: 'CVE-1', cvss: { nvd: { v3score: '7.2' } }, packageName: 'pkg', fixedVersions: ['1.0'], severity: 'high', suppressed: false, installedVersion: '1.0', purl: 'purl', description: 'd' } ] } ]
      }
    };
  ...
  test('fetch populates rows and registryCrds', async () => {
    const rows = [ {
      id: 'r1',
      imageMetadata: { registryURI: 'r1', repository: 'repo1', tag: 't1', registry: 'reg1' },
      report: { summary: { critical: 0, high: 0, medium: 0, low: 0, unknown: 0 } },
    ...
    const store: any = {
      ...
      dispatch: jest.fn((action: string, payload: any) => {
        if (action === 'cluster/findAll') {
          ...
          if (!store._called) { store._called = true; return Promise.resolve(rows); }
