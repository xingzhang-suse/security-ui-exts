...
import { SBOMSCANNER, SBOMSCANNER_REPOS, CNPG, CNPG_REPOS, CERT_MANAGER, CERT_MANAGER_REPOS } from '@pkg/types';
...
export default {
  ...
  watch: {
    controllerChart4Sbomscanner(oldVal, newVal) {
      if (oldVal?.id !== newVal?.id) {
        this.installSteps[2].ready = true;
        this.$refs.wizard?.goToStep(4);
      }
    },
    controllerChart4Cnpg(oldVal, newVal) {
      if (oldVal?.id !== newVal?.id) {
        if (this.installSteps[2].ready) {
          this.$refs.wizard?.goToStep(4);
          // this.addRepository4Cnpg();
        } else {
    ...
    controllerChart4CertManager(oldVal, newVal) {
      if (oldVal?.id !== newVal?.id) {
        if (this.installSteps[1].ready) {
          this.$refs.wizard?.goToStep(3);
          // this.addRepository4Cnpg();
        } else {
  ...
  computed: {
    ...
    sbomscannerRepo() {
      ...
      console.log('chart of sbomscannerRepo',chart)
      return this.repos?.find((repo) => repo.id === chart?.repoName);
    },

    cnpgRepo() {
      const chart = this.charts?.find((chart) => chart.chartName === CNPG.CONTROLLER);
      console.log('chart of cnpgRepo',chart)
      return this.repos?.find((repo) => repo.id === chart?.repoName);
    },

    certManagerRepo() {
      const chart = this.charts?.find((chart) => chart.chartName === CERT_MANAGER.CONTROLLER);
      console.log('chart of certManagerRepo',chart)
      return this.repos?.find((repo) => repo.id === chart?.repoName);
    },


    hasSbomscannerSchema() {
      console.log("hasSbomscannerSchema", this.$store.getters['cluster/schemaFor'](SBOMSCANNER.SCHEMA))
      return this.$store.getters['cluster/schemaFor'](SBOMSCANNER.SCHEMA);
    },

    hasCnpgSchema() {
      console.log("hasCnpgSchema", this.$store.getters['cluster/schemaFor'](CNPG.SCHEMA))
      return this.$store.getters['cluster/schemaFor'](CNPG.SCHEMA);
    },

    hasCertManagerSchema() {
      console.log("hasCertManagerSchema", this.$store.getters['cluster/schemaFor'](CERT_MANAGER.SCHEMA))
    ...
    async addRepository4CertManager(btnCb) {
      ...
      try {
        ...
        try {
          ...
          btnCb ? btnCb(false) : null;

          return;
        }
        if (!this.controllerChart4CertManager) {
          console.log("debouncedRefreshCharts(controllerChart4CertManager)")
    ...
    async addRepository4Cnpg(btnCb) {
      ...
      try {
        ...
        if (!this.controllerChart4Cnpg) {
          console.log("debouncedRefreshCharts(controllerChart4Cnpg)")
          this.debouncedRefreshCharts();
        }
    ...
    async addRepository4Sbomscanner(btnCb) {
      ...
      try {
        ...
        if (!this.controllerChart4Sbomscanner) {
          console.log("debouncedRefreshCharts(controllerChart4Sbomscanner)")
          this.debouncedRefreshCharts();
        }
      } catch (e) {
        handleGrowl({
    ...
    chartRoute() {
      ...
      if (!this.hasCertManagerSchema) {
        try {
          ...
          console.log('controllerChart4CertManager', this.controllerChart4CertManager)
          const latestChartVersion = getLatestVersion(this.$store, versions);
          console.log('latestChartVersion', latestChartVersion)
...
<template>
  ...
  <div v-else class="container">
    <div v-if="!install && !cnpgRepo && !sbomscannerRepo && !certManagerRepo" class="title p-10">
    ...
    <template v-else  style="display: flex">
      ...
      <template  style="display: flex">
        <InstallWizard ref="wizard" :init-step-index="initStepIndex" :steps="installSteps" data-testid="sb-install-wizard" style="width: 100%;">
          <template #repository4CertManager>
            ...
            <AsyncButton mode="certManagerRepository" data-testid="sb-repo-add-button" @click="addRepository4CertManager" />
          </template>
          <template #repository4Cnpg>
            ...
            <AsyncButton mode="cnpgRepository" data-testid="sb-repo-add-button" @click="addRepository4Cnpg" />
            <button class="btn role-secondary" style="margin-top: 16px;" @click="skip">Skip to Repository of Sbomscanner</button>
          </template>
          ...
          <template #repository4Sbomscanner>
            ...
            <div style="display: flex; align-items: center; gap: 24px;">
              <button class="btn role-secondary" @click="previous">Previous</button>
              <AsyncButton mode="sbomscannerRepository" data-testid="sb-repo-add-button" @click="addRepository4Sbomscanner" />
            </div>
          </template>
          ...
          <template #install>
            ...
            <div class="chart-route">
              <Loading v-if="!controllerChart4Cnpg && !controllerChart4Sbomscanner && !controllerChart4CertManager && !reloadReady" mode="relative" class="mt-20" />
