...
export default {
  ...
  computed: {
    hasSchema() {
      console.log("this.$store.getters['cluster/schemaFor'](RESOURCE.REGISTRY)", this.$store.getters['cluster/schemaFor'](RESOURCE.REGISTRY))
...
<template>
  <div v-if="allSchemas">
    <InstallView v-if="!hasSchema" :hasSchema="hasSchema"/>
    <Dashboard v-else />
  </div>
</template>
